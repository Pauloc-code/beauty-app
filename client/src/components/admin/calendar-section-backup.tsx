...
export default function CalendarSection() {
  ...
  // Filtrar todos os agendamentos primeiro
  const getFilteredAppointments = () => {
    ...
    if (filters.status !== "all") {
      filtered = filtered.filter((app: any) => app.status === filters.status);
    }
    if (filters.service !== "all") {
      filtered = filtered.filter((app: any) => app.serviceId === filters.service);
    }
    if (filters.client !== "all") {
      filtered = filtered.filter((app: any) => app.clientId === filters.client);
    }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <Card>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-3">
              ...
              >
                ...
                {showFutureAppointments ? "Ver Calendário" : "Ver Lista"}
              </Button>
                ...
                <DialogContent className="sm:max-w-md">
                  ...
                  {selectedAppointment && (
                    <div className="space-y-4 py-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <Input
                            id="edit-date"
                            type="date"
                            defaultValue={format(new Date(selectedAppointment.date), "yyyy-MM-dd")}
                      ...
                      <div>
                        <Label>Cliente</Label>
                        <Select value={selectedAppointment.clientId} onValueChange={(value) => setSelectedAppointment(prev => ({ ...prev, clientId: value }))}>
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <div>
                        <Label>Serviço</Label>
                        <Select value={selectedAppointment.serviceId} onValueChange={(value) => setSelectedAppointment(prev => ({ ...prev, serviceId: value }))}>
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <div>
                        <Label>Status</Label>
                        <Select value={selectedAppointment.status} onValueChange={(value) => setSelectedAppointment(prev => ({ ...prev, status: value }))}>
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
        ...
        <CardContent className="p-6">
          {/* Mostrar lista de agendamentos futuros ou calendário */}
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      {dayAppointments.slice(0, 2).map((appointment: any, index) => (
                        <div
                          key={appointment.id}
                          className="bg-primary text-white text-xs px-1 py-0.5 rounded truncate cursor-pointer hover:bg-primary/80"
                      ...
                      return (
                        <div key={`${day.toISOString()}-${hour}`} className="p-2 border-r border-gray-200 last:border-r-0 min-h-[60px]">
                          {dayAppointments.map((appointment: any) => (
                            <div
                              key={appointment.id}
                              className="bg-primary text-white text-xs px-2 py-1 rounded mb-1 truncate cursor-pointer hover:bg-primary/80"
                              title={`${format(new Date(appointment.date), "HH:mm")} - ${appointment.client.name}`}
                              onClick={() => {
                                setSelectedAppointment(appointment);
                                setIsEditAppointmentOpen(true);
